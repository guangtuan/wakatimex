...
- 应用配置：`src/wakatime_sync/sys/config.py`
- 数据模型：`src/wakatime_sync/sys/db.py`
- 前端页面与资源：`src/wakatime_sync/web/index.html`、`src/wakatime_sync/web/chart.html`、`src/wakatime_sync/web/assets/app.js`、`src/wakatime_sync/web/assets/styles.css`
...
- 时间窗口、日历分桶、小时分布等统计改动，优先保持前后端都走 `Asia/Shanghai` 同一口径
...
- 不要把设计过程、实现思路、方案说明、预览/实验性质提示直接写进页面文案；页面上的标题、副标题、说明文字必须站在最终用户视角，而不是开发者视角
- 日期是主视觉区域：顶部标题和摘要卡片要尽量紧凑，不抢月历空间
- 首页与图表页已拆分：`index.html` 聚焦月历，`chart.html` 承接语言 / 项目 / AI / 排行内容
- 月历规则：
  - 周一是每周第一天
  - 默认看当前月，可前后切月
...
- 当前月历交互是“左侧月历 + 右侧详情面板”；点击日期后，右侧展示当天 `Languages` / `Projects`，并补充 24h 活跃分布
